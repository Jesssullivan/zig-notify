...
def _add_platform_requirements(lines: list[str]) -> None:
    ...
    if has_macos:
        ...
        elif any("osascript" in f.read_text(encoding="utf-8", errors="ignore") for f in src_dir.glob("*_macos.zig")):
            lines.append("- No frameworks; uses the macOS osascript system binary")
...
def _add_link_frameworks(lines: list[str]) -> None:
    ...
    if has_macos or has_linux:
        lines.append("At final link time, Linux consuming applications must link libnotify/glib libraries.")
        lines.append("The macOS backend uses osascript and does not link Apple notification frameworks.")
        lines.append("")
